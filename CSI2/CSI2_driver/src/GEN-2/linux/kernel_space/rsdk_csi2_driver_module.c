        ...
        else
        {
            ...
            if (val != 0x7fffu)
            {
                (void)pr_err("RsdkCsi2Probe: ioremap error - phys=%lx, virt=%lx, test val=%x.\n",
                             (long)pRsdkCsi2Dev->dtsInfo.pMemMapBaseAddr, (long)pRsdkCsi2Dev->pMemMapVirtAddr, val);
                err = -EFAULT;
            ...
            {
#ifdef DEBUG_MODE
                (void)pr_alert("RsdkCsi2Probe: ioremap -> phys=%lx, virt=%lx.\n",
                               (unsigned long)pRsdkCsi2Dev->dtsInfo.pMemMapBaseAddr,
                               (unsigned long)pRsdkCsi2Dev->pMemMapVirtAddr);
#endif
            }
        }
        if (err == 0)
        {
            ...
            {  // map the memory only for the first unit
                ...
                (void)pr_alert("RsdkCsi2Probe: ioremap for DFS -> phys=%lx, virt=%lx.\n",
                               (unsigned long)DFS_PHY_ADDRESS, (unsigned long)DFS_MEM_LEN);
#endif
...
static int RsdkCsi2Remove(struct platform_device *pOfpDev)
{
    ...
    (void)RsdkCsi2RpcSrvExit(pRsdkCsi2Dev->dtsInfo.devId);
...
static int __init RsdkCsi2ModInit(void)
{
    ...
    {                   // platform_driver_register() OK, so only count the available units
        i = 0;
        while ((i < RSDK_MIPICSI2_INTERFACES) && (gpRsdkCsi2Device[i] != NULL))
        {

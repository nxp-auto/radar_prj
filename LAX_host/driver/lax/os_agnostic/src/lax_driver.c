...
static void WatchdogCallback(unsigned long data);
...
{
	int32_t ret = 0;
    EventListUpdate(&gOalCommVspaCtrl->vspaList);
	...
	switch (func) {
        ...
        case RSDK_LAX_UAPI_EVENT_READ:
        {
            uint32_t buf[sizeof(struct vspa_event)/sizeof(uint32_t) + 2];
            struct vspa_event* pEvent = (struct vspa_event *)buf;
            struct vspa_event_read* pEventRd = (struct vspa_event_read*)in;
            uint32_t validEvt = 0;
            int32_t oalret = 0;
     
            pr_debug("vspa%d: RSDK_LAX_UAPI_EVENT_READ \n", gOalCommVspaCtrl->id);
            if ((pEventRd->event_mask & VSPA_MSG_ALL) == VSPA_MSG_CMD)
            {
                CopyBytes((char*)pEvent, in + sizeof(struct vspa_event_read), sizeof(struct vspa_event));
            }
            ret = ReadEvent(&gOalCommVspaCtrl->vspaList, pEventRd, pEvent);
            pr_debug("ReadEvent mask %04X, ret= %d\n", pEventRd->event_mask, ret);
            if (ret > 0) 
            {
                validEvt = RSDK_LAX_UAPI_VALID;
            }
            oalret = OAL_RPCAppendReply(d, (char*)pEvent, sizeof(struct vspa_event));
            if (oalret != 0)
            {
                ret = -EFAULT;
                break;   
             }
             oalret = OAL_RPCAppendReply(d, (char*)&validEvt, sizeof(uint32_t));
             if (oalret != 0)
             {
                ret = -EFAULT;
             }
...
static int32_t VSPAPowerUp(vspaControl_t *pVspaCtrl)
{
    ...
    if (pVspaCtrl->state != VSPA_STATE_POWER_DOWN)
        return -EPERM;

    /* Disable all interrupts */
    VspaRegWrite(pRegs + IRQEN_REG_OFFSET, 0x0);
    DmaResetQueue(pVspaCtrl);
    EventResetQueue(&pVspaCtrl->vspaList);
    MboxReset(pVspaCtrl);
    CmdReset(pVspaCtrl);

    pVspaCtrl->state = ret ? VSPA_STATE_UNKNOWN :
...
static int32_t StartUp(vspaControl_t *pVspaCtrl)
{
    ...
    if (pVspaCtrl->state != VSPA_STATE_LOADING)
        return -EPERM;
    ...
    pVspaCtrl->watchdogValue = ~(VspaRegRead(pRegs + GP_OUT3_REG_OFFSET) >> 16);
    pVspaCtrl->state = VSPA_STATE_RUNNING_IDLE;
    VspaEnableMailboxIrqs(pVspaCtrl);
    OAL_init_completion(&pVspaCtrl->watchdogComplete);
    WatchdogCallback((unsigned long) pVspaCtrl);
    return 0;

startup_fail:
    pVspaCtrl->versions.vspa_sw_version = ~0;
    pVspaCtrl->versions.ippu_sw_version = ~0;
    pVspaCtrl->state = VSPA_STATE_STARTUP_ERR;
...
static void VspaFlags0IrqHandler(vspaControl_t *pVspaCtrl)
{
    ...
    while (flags0) {
        ...
        /* Optionally send CMD consumed signal/event message */
        if (pVspaCtrl->seqId[seqId].flags & VSPA_FLAG_REPORT_CMD_CONSUMED)
        {
            if ((pVspaCtrl->flags & RSDK_LAX_SEND_COMPLETION_MASK) == RSDK_LAX_SEND_COMPLETION_SIGNAL)
            {
            ...
            {
                EventEnqueue(&pVspaCtrl->vspaList,
                      VSPA_EVENT_CMD,
                      pVspaCtrl->seqId[seqId].cmdId, 0,
                      pVspaCtrl->seqId[seqId].payload1,
                      0);
            }
        }
    ...
    if (flags) {
        ...
        if (pDr->type == VSPA_EVENT_DMA) {
            ...
            if (pDr->flags & VSPA_FLAG_REPORT_DMA_COMPLETE || err) {
                EventEnqueue(&pVspaCtrl->vspaList, VSPA_EVENT_DMA, pDr->id, err, pDr->dmem_addr, pDr->byte_cnt);
            }
...
static void WatchdogCallback(unsigned long data)
{
    vspaControl_t *pVspaCtrl = (vspaControl_t *)data;
    uint32_t  *pRegs = pVspaCtrl->pRegs;
    uint32_t val;

    if (pVspaCtrl->state == VSPA_STATE_RUNNING_IDLE &&
        pVspaCtrl->watchdogIntervalMsecs > 0) {
        val = VspaRegRead(pRegs + GP_OUT3_REG_OFFSET) >> 16;
        IF_LAX_DRV_DEBUG(DEBUG_WATCHDOG)
            pr_info("vspa%d: watchdogValue = %04X\n",
                            pVspaCtrl->id, val);
        if (val == pVspaCtrl->watchdogValue) {
            EventEnqueue(&pVspaCtrl->vspaList, VSPA_EVENT_ERROR,
                    VSPA_ERR_WATCHDOG, 0, val, 0);
        }
        pVspaCtrl->watchdogValue = val;
        val = VspaRegRead(pRegs + CONTROL_REG_OFFSET);
        val = (val & CONTROL_REG_MASK) | CONTROL_HOST_GO;
        VspaRegWrite(pRegs + CONTROL_REG_OFFSET, val);
/*        OAL_mod_timer(&pVspaCtrl->watchdogTimer, jiffies +
            msecs_to_jiffies(pVspaCtrl->watchdogIntervalMsecs));*/

        OAL_set_timer_timeout(&pVspaCtrl->watchdogTimer, pVspaCtrl->watchdogIntervalMsecs * USEC_IN_MSEC);
        OAL_add_timer(&pVspaCtrl->watchdogTimer);
    } else {
        OAL_complete(&pVspaCtrl->watchdogComplete);
    }

    EventListUpdate(&pVspaCtrl->vspaList);
}
...
int32_t VSPAPowerDown(vspaControl_t *pVspaCtrl)
{
    ...
    pVspaCtrl->versions.vspa_sw_version = ~0;
    pVspaCtrl->versions.ippu_sw_version = ~0;
    pVspaCtrl->watchdogIntervalMsecs = VSPA_WATCHDOG_INTERVAL_DEFAULT;
    ...
    if (pVspaCtrl->state == VSPA_STATE_RUNNING_IDLE) {
        ...
        if (!(ret1)) {
            LAX_DRV_ERR("%d: PowerDown() timeout waiting for halt\n",
                    pVspaCtrl->id);
            ret = -ETIME;
            pVspaCtrl->state = VSPA_STATE_UNKNOWN;
            return ret;
        }
    ...
    /* shut the timer down */
    /*OAL_mod_timer(&pVspaCtrl->watchdogTimer, jiffies);*/
    OAL_set_timer_timeout(&pVspaCtrl->watchdogTimer, 0);
    OAL_add_timer(&pVspaCtrl->watchdogTimer);

    pVspaCtrl->state = ret ? VSPA_STATE_UNKNOWN : VSPA_STATE_POWER_DOWN;
    return ret;
} 
...
int32_t VspaDmaRequest(vspaControl_t *pVspaCtrl, struct vspa_dma_req * pDmaReq)
{
    int32_t rc = 0;
    uint32_t alignBytes = pVspaCtrl->hardware.axi_data_width >> 3;
    if (pVspaCtrl->state == VSPA_STATE_UNPROGRAMMED_IDLE) {
        pVspaCtrl->state = VSPA_STATE_LOADING;
        /* Enable DMA error and DMA complete interrupts */
        VspaEnableDmaIrqs(pVspaCtrl);
    } else if (pVspaCtrl->state < VSPA_STATE_LOADING)
        return -EPERM;
...
int32_t VspaCmdSend(vspaControl_t *pVspaCtrl, char* pCmd, uint32_t len)
{
    ...
    pVspaCtrl->seqId[seqId].cmdId = seqId;
...
{
    int state = pVspaCtrl->state;

    if (state == VSPA_STATE_UNPROGRAMMED_IDLE ||
        state == VSPA_STATE_RUNNING_IDLE) {
        if (VspaRegRead(pVspaCtrl->pRegs + STATUS_REG_OFFSET) & 0x100)
            state++;
    }
    return state;
}
...
void VspaInit(vspaControl_t *pVspaCtrl)
{
    ...
    vspaEventList_t * pVspaList = &pVspaCtrl->vspaList;


    dmaSize = pVspaCtrl->cmdBufferBytes;
    pVspaCtrl->state = VSPA_STATE_UNKNOWN;
//    pVspaCtrl->debug = 0xFFFFFFFF;
    ...
    OAL_setup_timer(&pVspaCtrl->watchdogTimer, WatchdogCallback,
                        (unsigned long)pVspaCtrl, 0);

    OAL_spin_lock_init(&pVspaCtrl->dmaTxQueueLock);
    OAL_spin_lock_init(&pVspaCtrl->dmaEnqueueLock);
    OAL_spin_lock_init(&pVspaList->eventListLock);
    OAL_spin_lock_init(&pVspaList->eventQueueLock);
    OAL_spin_lock_init(&pVspaCtrl->mb64Lock);
    OAL_spin_lock_init(&pVspaCtrl->mbChkLock);
    OAL_spin_lock_init(&pVspaCtrl->controlLock);
    pVspaList->pollMask = VSPA_MSG_ALL;

    OAL_init_waitqueue(&pVspaList->eventWaitQ);
    EventResetQueue(pVspaList);
    DmaResetQueue(pVspaCtrl);
    MboxReset(pVspaCtrl);
    CmdReset(pVspaCtrl);
...
irqreturn_t VspaIrqHandler(int irq, vspaControl_t *pVspaCtrl)
{
    ...
    /*TODO define some constants for irqreturn_t */
    return 0;
}
...
void VspaShutdownTimer(vspaControl_t *pVspaCtrl)
{
    ...
    OAL_set_timer_timeout(&pVspaCtrl->watchdogTimer, 0);
    OAL_add_timer(&pVspaCtrl->watchdogTimer);
    OAL_wait_for_completion(&pVspaCtrl->watchdogComplete);

    OAL_del_timer(&pVspaCtrl->watchdogTimer);
}
...
{
        OAL_spin_lock(&pVspaCtrl->vspaList.eventListLock);
        pVspaCtrl->state = VSPA_STATE_RECOVERY;
        OAL_spin_unlock(&pVspaCtrl->vspaList.eventListLock);
        OAL_wake_up_interruptible(&pVspaCtrl->vspaList.eventWaitQ);
}

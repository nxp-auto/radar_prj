 ...
 *
 * @param[in] wq: The waitqueue
 * @param[in] condition: The expression to be evaluated
 * @param[in] timeout: The timeout in jiffies
 *
 * @return 0, if the condition evaluated to false after the timeout elapsed,
 *         1, if the condition evaluated to true after the timeout elapsed,
 *         the remaining jiffies,  (at least 1) if the condition evaluated
 *          to true before the timeout elapsed.
 */
#define OAL_wait_event_interruptible_timeout(wq, condition, timeout) \
	os_oal_wait_event_interruptible_timeout(wq, condition, timeout)
